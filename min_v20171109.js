function boot() {
    ...
    this.allPage = [];//请帖单页
    this.width = window.innerWidth;         //设备宽度
    this.height = window.innerHeight;       // 设备高度   
    ...
    this.giftProperty = {};
    this.send_state = 0;
    this.Host = "https://access.djtaoke.cn/socket/response.do";     //主地址
    this.send_name = '';
    this.send_price = '';
...
boot.prototype = function () {
    ...
    ajax_info = function () {                                              // 获取页面信息
        ...
        if($this.getParams('edit')==1){     // 编辑状态显示编辑栏
            $this.editState = true;
            $this.isshowEdit();
        }


        this.card_id = this.getParams('card_id');        // 获取url中的card_id参数值
        this.type = this.getParams('type');              // 获取去url中的type参数值
        this.Ttype = this.getParams('Ttype');              // 获取去url中的Ttype参数值 请求的模版的类型 1，请帖模版 2，个人请帖
        this.user_id = this.getParams('userId');          //获取url中的userId参数值， Ttype为2时必须
        this.wedding_id = this.getParams('weddingId');     // 获取url中的weddingId参数值， Ttype为2时必须
        $.ajax({                                         // 获取请帖数据通过 card_id获取
            url: $this.Host,
            type: 'POST',
            headers:{
                'ServerName': 'wedding',
                'MethodName': 'getCardTemplateInfo'
            },
            contentType: 'application/json',
            data: JSON.stringify({
                action_name: 'get_card_template_info',
                data:{
                    template_id: $this.card_id,
                    type: $this.Ttype,
            ...
            success: function (res) {
                ...
                if (res.code== 0) {
                    ...
                    $this.init();  //TODO: 初始化数据
                    if ($this.getCookie('musicStatePause')) { //TODO: 开启背景音乐
                        $this.musicStatePause = $this.getCookie('musicStatePause')
                    }
                    ...
                    if (res.data.music.audio) {
                        ...
                        if($this.getParams('imgStr')){
                            $this.editInfo()                             // 修改婚礼信息
                        }

                        //TODO: 初始化音乐图标
                        var _div = document.createElement('div'), _img = document.createElement('img'),              //创建div 和img标签
                            _on = $this.musicOpen && $this.musicOpen != "" ? $this.musicOpen : 'http//qnm.hunliji.com/o_1bi67lq091qtt1gfs60cpadqjj7.png',
                            _off = $this.musicClose && $this.musicClose != "" ? $this.musicClose : 'http//qnm.hunliji.com/o_1bi67m2q63tilg81vh1q3v10g6c.png';
                        ...
                        $(document).on('touchstart', '#musicBtn', function () {
                            ...
                            if (_this.hasClass('rotate')) {
                                ...
                                $this.writeCookie('musicStatePause', 'true', 360)      // 将音乐状态和信息放到cookie中
                                document.getElementById('playMusic').pause()
                                _this.removeClass('rotate')                            // 移除旋转动画
                                $('#musicBtn img').attr('src', _off)                   // 使用关闭音效图片
                            } else {
                                $this.musicStatePause = 'false';
    ...
    ISedit = function(){
       ...
       _div.setAttribute('id', 'edit_page') ;
        document.body.appendChild(_div);
       $('#edit_page').append('<div class="" style="width:1px;height:40px;"></div>')
       $('#edit_page').append('<div class="operation" id="add_page"><img src="https://oss.ririyuedu.com/tmp/wxb2de1aacb42e47df.o6zAJs6e4iQOHuo7DT2vSURT-cTg.d5b9c6c9941c252f7d6b64afb576c066.png" style="width:'+ 44*sm +'px;height:'+ 49*sm +'px;"/><div class="tab_title">加页</div></div>')
       $('#edit_page').append('<div class="operation" id="sort_page"><img src="https://oss.ririyuedu.com/tmp/wxb2de1aacb42e47df.o6zAJs6e4iQOHuo7DT2vSURT-cTg.d09864585482b15f523e479a3c4cd079.png" style="width:'+ 49*sm +'px;height:'+ 40*sm +'px;"/><div class="tab_title" style="margin-top: '+ 15*sm +'px;">排序/删页</div></div>')
       $('#edit_page').append('<div class="operation" id="preview_all"><img src="https://oss.ririyuedu.com/tmp/wxb2de1aacb42e47df.o6zAJs6e4iQOHuo7DT2vSURT-cTg.f7496f2d469f58d7863c587e7e280883.png" style="width:'+ 44*sm +'px;height:'+ 49*sm +'px;"/><div class="tab_title">预览</div></div>')
       $('#edit_page').append('<div class="operation" id="setting_info"><img src="https://oss.ririyuedu.com/tmp/wxb2de1aacb42e47df.o6zAJs6e4iQOHuo7DT2vSURT-cTg.ddea163d7ceb975737bd1a6c55a8d2b2.png" style="width:'+ 49*sm +'px;height:'+ 49*sm+'px;"/><div class="tab_title">设置</div></div>')
       $('#edit_page').append('<div class="send" id="sendTo"><img src="https://oss.ririyuedu.com/tmp/wxb2de1aacb42e47df.o6zAJs6e4iQOHuo7DT2vSURT-cTg.05c7cc48ece8738d30739ff03a2051ad.png" style="width: 30px;height:30px;"/><div class="">发送</div></div>')
    ...
    isshowEdit = function () {
      var $this = this;
      var _div = document.createElement('div');
      _div.setAttribute('id', 'show_edit');
      _div.innerHTML='<img src="http://7xld1x.com1.z0.glb.clouddn.com/set.png" style="width:30px;height:30px;"/>';
      document.body.appendChild(_div);
    }
    ...
    editInfo = function () {
        ...
        // var location;
        // for()
        // var srcs = JSON.parse($this.getParams('imgStr'));
        // pageList = $this.result.page;
        // for(var i = 0; i< pageList ; i++){
        //     elementList = pageList[i].layout.elements;
        //     for(var j=0; j< elementList.length; j++){
        //         if(elementList[j].text_type == 'groom'){
    ...
    init = function () {
        ...
        this.result.page.forEach(function (page, i) {
            if (page.layout.layTemplate) {        //判断是不是最后一张带有地址的页面
                $this.guests.no = i;
                $this.guests.html = page;                              // 最后一张页面内容
                $this.buttonBg = page.layout.attendButton.boxColor;    // 参加按钮样式
                $this.textColor = page.layout.attendButton.textColor;  // 参加按钮文字样式
            }
        })
        $this.result.guest_template == 0 ? $this.guestPageHide(true) : null;          // 判断是否显示宾客页面
        $this.musicOpen = $this.result.music.open_img || null;
        ...
        for (var i = 0; i < _ele.length; i++) {
            _imgs.push(_ele[i].img)                           //第一个页面的所有的图片放到_imgs中
        }
        ...
        this.loading(_imgs, function () {
            ...
            // setTimeout(function () {
            //     1 == $("all-page div.layout").length && $("all-page").append($this.createPage($this.result.page).slice(1))
            // }, 300)
            ...
            setTimeout(function() {
				$this.getCookie("editState") && $this.type && ($this.editState = "true" == $this.getCookie("editState") ? !0 : !1), 
                $this.type && $this.getParams("edit") && ($this.editState = 1 == $this.getParams("edit") ? !0 : !1), 
				$this.editIconState($this.editState)
			}, 3600)
			// $this.sdk(), 
			// $this.is_get_confirm || (
			//    sessionStorage.getItem("gift_id") && $this.wxsq_event("CardGift2Recv", sessionStorage.getItem("gift_id")), 
            //    sessionStorage.getItem("cash_id") && $this.wxsq_event("CardGift2Recv", sessionStorage.getItem("cash_id"))
            // )
			// setTimeout(function() {
			//    sessionStorage.getItem("current_seating") && ($this.gotoPage(sessionStorage.getItem("current_seating")), $this.chatMsg()), $this.giftPage()
            // }, 100)
...
//$this.changeVideo('http://qnm.hunliji.com/o_1bk6op88ds1mctv0d1nkp1qqa7.mp4','544','960')
        ...
        $(document).on('touchstart', '.editIcon', function () {
            ...
            if (_this.hasClass('card_info')) {
                $this.editCard_app('words', {cardInfo: true})
            } else {
                $this.editCard_app('img', obj);
            }
        ...
        $(document).on('touchstart', '#show_edit', function () {    //显示功能栏
	   console.log('dianji operation')
            $('#show_edit').hide();
            if(document.getElementById('edit_page')){
                ...
                var _div = document.getElementById('edit_page');
                _div.setAttribute('class', 'showOP');
        })

        $(document).on('touchstart', '#wrap', function() {       //隐藏功能栏
            if(document.getElementById('edit_page')){
                var _div = document.getElementById('edit_page');
                _div.setAttribute('class', 'hideOP');
                setTimeout(function() {
                    $('#edit_page').hide();
                    $('#show_edit').show();
                },500)
        ...
        //功能栏中的点击事件
        $(document).on('touchstart', '#add_page', function () {
            wx.miniProgram.navigateTo({url:'/pages/Main_sonPage/AddCardPage/add_card_page'})
        })

        $(document).on('touchstart', '#sort_page', function () {
            // alert('页面排序');
            if(document.getElementById('MASK') && document.getElementById('page_list')){
                $('#MASK').show();
                $('#page_list').show();
            } else {
              $this.showList();
            }
            var move = document.getElementById('page_list');
            move.setAttribute('class', 'showList')
        })

        //预览 事件
        $(document).on('touchstart', '#preview_all', function () {
            if($this.user_id && $this.wedding_id){
                window.location.href="https://logic.hunlibaoapp.com/wedding/wedding.html?Ttype=2&card_id="+$this.card_id+'&userId=' + $this.user_id +'&weddingId=' + $this.wedding_id
            } else {
                window.location.href="https://logic.hunlibaoapp.com/wedding/wedding.html?Ttype=1&card_id="+$this.card_id; 
            }
            
        })

        $(document).on('touchstart', '#setting_info', function () {
            wx.miniProgram.navigateTo({url:'/pages/Main_sonPage/SetingCard/setting_card'})
        })

        $(document).on('touchstart', '#sendTo', function () {
            alert('发送');
        })
        ...
        $(document).on('touchstart', '#MASK', function () {
            var _hide = document.getElementById('page_list');
            _hide.setAttribute('class', 'hideList');
            setTimeout(function() {
                $('#MASK').hide();
                $('#page_list').hide();
            },500 )
        ...
        $(document).on('click', '#sortList', function(e){
            var n = e.srcElement.id;
            for(var i=0; i< $this.result.page.length; i++){
                document.getElementById('sortList').getElementsByTagName('img')[i].style.border = '1px #ccc solid';
            }
            document.getElementById('sortList').getElementsByTagName('img')[n].style.border="1px red solid";
            $this.gotoPage(n)
        })

        $(document).on('touchstart', '#sortList', function(e){
    ...
    exchange = function(){
            ...
            var sortable = Sortable.create(el, {
                ...
                onSort: function (evt){
                    var list = [];
                    var sortlist = evt.from.children;
                    for(var i = 0; i < sortlist.length; i++){
                        list.push(sortlist[i].id);
                    }
                    console.log(list);
                },
    ...
    showList = function () {
        ...
        var _div = document.createElement('div');
        _div.setAttribute('id', 'page_list');
        document.body.appendChild(_div);
        document.body.appendChild(mask);
        var length = this.result.page.length;
        $('#page_list').append('<div class="" style="width:100%;height:35px;line-height:35px;margin-left:10px;font-weight:500;">长按拖动排序</div>')
        for(var i=0; i< this.result.page.length; i++){
            list += '<img src="'+ this.result.page[i].layout.cover_pic +'" id ="'+ i +'" class="toolbar" style="width:'+ this.width*0.23 +'px;height:'+this.height*0.23+'px;margin-right:5px;"/>'
        }
        var allpage = '<div class="sortList" id="sortList" style="overflow:auto;height:' + this.height * 0.23 +'px;"><nobr id="items">'+ list + '</nobr></div>'
    ...
    chatMsg = function ($state) {
        ...
        if ($this.seating > 0 && $this.seating <= $this.result.page.length - 1 && !$('.layout').eq($this.seating).hasClass('guestPage')) {
            ...
            if ($('#giftModule').length <= 0) {
                setTimeout(codeGift, 600)
            }
        ...
        function codeGift($state) {
            if ($this.gift_m != 0 || $this.seating <= 0 || $this.seating >= $this.result.page.length - 1) {
                return false
            }
    ...
    allImg = function (result) {
        ...
        var _imgs = [];           // 初始化的两张图片 一张是二维码， 一张是婚礼纪logo
    ...
    add_infinite = function () {
        console.log("add_infinite");
        var $this = this, _infArr = this.infiniteArr, _infArrW = _infArr.length;
        var _anis = $('.animated');
        console.log(_infArr)
        $this.timeAni ? clearTimeout($this.timeAni) : null;
        for (var i = 0; i < _infArrW; i++) {
            if (_infArrW[i].id == 'ele_7') {
                console.log(_infArrW[i].delay, _infArrW[i].duration)
            }
    ...
    createPage = function (arr) {
        ...
        return this.allPage = arr.map(function (v, i) {
            ...
            v.layout.elements.forEach(function (a, b) {
                ...
                if (a.img == 'http://qnm.hunliji.com/o_1bng0elfb4251up11lqk11r47ih4t.png') {
                    console.log(obj)
                }
                ...
                var isDown = obj.isdown ? 'bottom:' + bottom : 'top:' + obj.top,
                    ...
                    // console.log(a,b, v.layout.elements.length);
                ...
                var animateName = {
                        'pullDown': 'fadeInDown',
                        'fadeInNormal': 'fadeIn',
                        'stretchLeft': 'bounceInLeft',
                        'stretchRight': 'bounceInRight'
                    }[a.animate] || a.animate;
    ...
    guestsPage = function (v, i) {
        ...
        var _addr = '';
        $this.mapUrl = '//apis.map.qq.com/tools/poimarker?type=0&marker=coord:' + obj.x + ',' + obj.y + ';coordtype:3;title:杭州锐傲;addr:' + _addr + '&key=GMZBZ-ZCD3U-GY3VD-4PJK3-BRTK3-SQFWZ&referer=hunliji'
    ...
    editCard_app = function ($state, obj) {
        ...
        var edit = this.getParams('edit');
        var Pagearr = this.result.page[this.seating].layout.elements;
        if ($state == 'words') {
            if(obj.cardInfo){
                wx.miniProgram.navigateTo({
                  url:'/pages/Main_sonPageSetingCard/setting_card?currentPage='+this.seating + '&id='+obj.id
                })
            }
        } else {
            ...
            wx.miniProgram.navigateTo({
              url:'/pages/Main_sonPage/ImgHtml/img_html?currentPage='+this.seating + '&id='+obj.id
            })
        }
    }
    ...
    positionIcon = function (obj, isDown, v, i, ws) {
        ...
        if (v.text_type == 'time' || v.text_type == 'bride' || v.text_type == 'groom' || v.text_type == 'location') {
            $this.log.push({
            ...
            if (i == ws - 1) {
                ...
                for (var r = 0; r < this.log.length; r++) {
                    if(this.log[r].text_type == 'time'){
                      wf.x = Number(this.log[r].x)
                      wf.y = Number(this.log[r].y)
                    }else {
                ...
                var _style = {
                    top: state,
                    left: Number(wf.x / $this.UI_WIDTH) * width + Number(wf.w / $this.UI_WIDTH) * width / 2 - 20 + 'px'       // Number($this.width / 2 - 20) + 'px'//
                }
                var _edit = '<div class="editIcon edit_text card_info hide" page_id="' + obj.page_id + '" id="' + obj.id + '" type="' + obj.type + '" style="' + _style.top + ';left:' + _style.left + '"><i></i><em></em><img src="http://7xld1x.com1.z0.glb.clouddn.com/make_19.png" /></div>';
                $('#other').append(_edit)
        ...
        if (obj.type == 'image') {
            ...
            var _edit = '<div  class="editIcon edit_img hide ' + (obj.type == 'map' ? "card_info" : "") + ' ' + (v.video_width ? "videoIcon" : "") + '" videoW="' + v.video_width + '" videoH="' + v.video_height + '" videoP="' + v.video_path + '" page_id="' + obj.page_id + '" id="' + obj.id + '" type="' + obj.type + '" style="' + _style.top + ';left:' + _style.left + '"><i></i><em></em><img src="http://7xld1x.com1.z0.glb.clouddn.com/make_07.png" /></div>';
            ...
            var _edit = '<div  class="editIcon edit_img hide ' + (obj.type == 'map' ? "card_info" : "") + ' ' + (v.video_width ? "videoIcon" : "") + '" videoW="' + v.video_width + '" videoH="' + v.video_height + '" videoP="' + v.video_path + '" page_id="' + obj.page_id + '" id="' + obj.id + '" type="' + obj.type + '" style="' + _style.top + ';left:' + _style.left + '"><i></i><em></em><img src="http://7xld1x.com1.z0.glb.clouddn.com/make_19.png" /></div>';
    ...
    lastAbout = function () {
        if ($('#lastAbout').length > 0 || this.type) {
            return false
        }
        this.lastState = true;
        this.lastDown = true
        ...
        $('#lastAbout').css({
            'top': this.height + 'px',
            'padding-top': 116 / this.UI_WIDTH * this.width + 'px',
            'width': this.width + 'px',
        ...
        $('.moreIcon').css({
            'marginTop': 90 / this.UI_WIDTH * this.width + 'px',
            'width': obj.towImg_w / this.UI_WIDTH * this.width + 'px'
        })
        $('#lastAbout h5').css({
    ...
    upDownIcon = function () {
        ...
        _animateIcon.innerHTML = '<img src="https://qnm.hunliji.com/o_1agpam0fsibn2814j110101jcr7.png"/>'
    ...
    return {
        ...
        ISedit: ISedit,//当为编辑页面时做的事
        isshowEdit: isshowEdit,
        showList: showList,  //显示当前请帖所有页面的列表。在排序功能中实现
        editInfo: editInfo,  //修改婚礼信息
    }
}()
